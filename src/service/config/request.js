...
export const getRequest = async (getnotification, headers = _authorizationHeaders()) => {
  try {
    const res = await axios.get(API_URL + getnotification, {
      headers: Object.assign({}, headers)
    });
    var users =  res.data.data ? res.data.data : res.data;
    console.log("NOti" + JSON.stringify(users));
  } catch (err) {
    handleError(err);
  }
};
